  ...
  if (connection && typeof connection.fetch === 'object' && typeof connection.fetch.register === 'function') {
    ...
    registerRoute({
      ...
      fetch: async (request) => {
        ...
        // The Studio frame runs in the browser, which may reach Archon at a
        // different address than the dsh host does (remote GUI, containers).
        const browserBase = process.env.DSH_ARCHON_BROWSER_URL || base
        let reachable = false
        let archon = null
        try {
        ...
        return json(200, { ok: true, archonBaseUrl: base, archonBrowserUrl: browserBase, reachable, archon, compat })
      },
    })
