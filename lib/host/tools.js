...
const CONTROL_VERBS = ['approve', 'reject', 'resume', 'cancel', 'abandon']
...
export function registerTools(ctx) {
  ...
  tools.register({
    name: 'archon_status',
    description:
      'Report whether the Archon workflow engine is reachable and its version, status, and active platforms. '
      + 'No arguments. Use before archon_workflows/archon_runs/archon_run so you know the server is up.',
    ...
    async execute() {
      ...
      return JSON.stringify({
        ...
        runningWorkflows: result.runningWorkflows ?? null,
      }, null, 2)
    },
  })
  ...
  tools.register({
    ...
    output: textOutput,
    async execute(args) {
      const result = await listWorkflows((args && args.cwd) || undefined)
      const list = (result.workflows || []).map((entry) => {
  ...
  tools.register({
    ...
    async execute(args) {
      ...
      const runs = (result.runs || []).map((run) => ({
        ...
        lastActivity: run.last_activity_at ?? null,
      }))
  ...
  tools.register({
    ...
    async execute(args) {
      ...
      if (codebasePath) {
        ...
        const match = (codebases || []).find((cb) =>
          cb.default_cwd === codebasePath || cb.path === codebasePath || cb.name === codebasePath)
        if (!match) {
  ...
  tools.register({
    ...
    async execute(args) {
      ...
      const payload = action === 'reject' ? { reason: (args && args.reason) || undefined }
        : action === 'approve' ? { comment: (args && args.comment) || undefined }
          : {}
