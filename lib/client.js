...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * body, mirroring the engine's node base schema. The importer does not partition
     * by this list — it removes the keys each variant consumes and keeps everything
     * else as `base`, so engine-only extras (`description`, `pi`, `mutates_checkout`,
     * `settingSources`, …) survive a round trip. The list drives the inspector: the
     * fields it can edit, and the count of preserved-but-uneditable ones.
     */
    var BASE_FIELD_KEYS = [
    ...
    /** Depth-first three-color walk reporting each `depends_on` back edge once. */
    ...
    /** Human text for a failed write (postJson/putJson resolve, never reject). */
    ...
    function studioPositions(s, edges) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        if (!positions[nodes[i].id]) return layoutGraph(nodes, edges);
      }
      return positions;
    }
    ...
    function ArchonStudio(props) {
      ...
      function openWorkflow(entry) {
        ...
        Archon.workflowDefinition(entry.name, cwd, source)
          ...
          .then(function (res) {
            if (!res || res.__error) {
              var message = res && res.__error && res.__error.message ? String(res.__error.message) : "";
              patch({
                busy: "",
                notice: message.indexOf("404") !== -1
                  ? "Not editable: the server does not serve workflows stored in a subdirectory of .archon/workflows."
                  : "Could not open " + entry.name + ": " + errorText(res && res.__error) + ".",
              });
              return;
            }
      ...
      function runValidation() {
        ...
        Archon.validateDefinition(definitionFor(name)).then(function (res) {
      ...
      function saveWorkflow(name, source) {
        ...
        Archon.validateDefinition(definition).then(function (res) {
          if (!res || res.valid !== true) {
            patch({
              busy: "",
              serverIssues: res && res.__httpStatus >= 200 && res.__httpStatus < 300
                ? serverIssues(res.errors)
                : serverIssues([studioWriteError(res)]),
              notice: "Not saved — the server rejected the definition.",
            });
            return;
          }
          Archon.saveWorkflow(name, scope.cwd, scope.target, definition).then(function (write) {
            if (!write || write.__httpStatus < 200 || write.__httpStatus >= 300) {
      ...
      function renameWorkflow(to) {
        ...
        Archon.saveWorkflow(to, scope.cwd, scope.target, definitionFor(to)).then(function (write) {
          if (!write || write.__httpStatus < 200 || write.__httpStatus >= 300) {
            patch({ busy: "", nameError: "Rename failed: " + studioWriteError(write) + "." });
            return;
          }
          Archon.deleteWorkflow(from, scope.cwd, scope.target).then(function (removed) {
            var failed = !removed || removed.__httpStatus < 200 || removed.__httpStatus >= 300;
            clearDirty({
              busy: "",
      ...
      function deleteWorkflow() {
        ...
        patch({ busy: "delete" });
        Archon.deleteWorkflow(name, scope.cwd, scope.target).then(function (res) {
          if (!res || res.__httpStatus < 200 || res.__httpStatus >= 300) {
      ...
      function addFromPalette(variant) {
        ...
        var positions = Object.assign({}, s.positions);
        var lowest = 0;
      ...
      function selectNode(id) {
        if (s.pendingConnect && s.pendingConnect !== id) {
      ...
      function startDrag(id, event) {
        ...
        var start = s.positions[id] || { x: 0, y: 0 };
        ...
        function move(moveEvent) {
          ...
          set(function (prev) {
            var positions = Object.assign({}, prev.positions);
            positions[id] = next;
    ...
    function renderStudioEditor(s, h) {
      ...
      return React.createElement("div", { className: "dsha-studio-editor" },
        ...
        React.createElement("div", { className: "dsha-studio-body" },
          ...
          renderStudioCanvas(s, h, edges, positions),
    ...
    function renderStudioCanvas(s, h, edges, positions) {
      ...
      if (variant === "loop") {
        ...
        return [
          studioField("Prompt source", React.createElement("select", {
            ...
            disabled: disabled,
            onChange: function (e) {
              // Exactly one source may be present: drop both, then set the chosen one.
              var cleared = setDataField(setDataField(s.model, node.id, ["loop", "prompt"], undefined), node.id, ["loop", "command"], undefined);
              h.patch({ model: setDataField(cleared, node.id, ["loop", e.target.value], ""), dirty: true });
            },
          }, [
      ...
      if (variant === "approval") {
        ...
        return [
          ...
          approval.decisions
            ...
            : null,
          approval.on_reject
            ? studioText(s, h, node, "On reject: rework prompt", ["approval", "on_reject"], false, true)
            : null,
        ];
      }
      if (variant === "wait") {
