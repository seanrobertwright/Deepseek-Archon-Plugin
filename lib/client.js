...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ArchonStudio(props) {
      ...
      /**
       * Re-read the workflow list for the current project (after a write). A
       * failure is reported in `wfError` rather than swallowed, and a reply that
       * arrives after the user switched projects is dropped: the effect above
       * already fetched the new project's list.
       */
      function refreshList() {
        ...
        return Archon.workflowsFor(studioCwd(s)).catch(function (e) { return { __error: e }; }).then(function (list) {
          set(function (prev) {
            ...
            if (!list || list.__error) {
              return Object.assign({}, prev, { wfError: "Workflows unavailable: " + errorText(list && list.__error) + "." });
            }
            return Object.assign({}, prev, { wfList: list, wfError: "" });
          });
          return list;
        });
      }
      ...
      /**
       * The editor session counter: a stable box whose `id` is bumped every time
       * the editor opens a workflow or returns to the picker. A Validate or Save
       * reply captures the id it started under and is dropped when it lands in
       * a different session (after ‹ Back, or after another workflow opened), so
       * a late reply never paints a stale notice or issues onto the picker or a
       * different workflow. Kept outside React state so the check is synchronous.
       */
      var session = React.useState({ id: 0 })[0];
      function nextSession() { session.id += 1; return session.id; }

      /** Apply `p` only while `token` is still the current editor session. */
      function patchIfOpen(token, p) {
      ...
      /**
       * The `cwd` a definition READ carries, for both the initial open and the
       * re-read after a save. The server resolves a name project-first, then
       * `~/.archon/workflows`, then bundled — under the given cwd, or under the
       * first registered codebase when none is sent. A global entry therefore
       * still carries the selected project's cwd: that is the checkout the list
       * was discovered for, so the lookup mirrors discovery exactly, whereas an
       * omitted cwd could let a same-named workflow in some other project shadow
       * it. Bundled entries send no cwd: nothing project-side is expected.
       * Writes are scoped separately by `studioSaveScope` (global writes never
       * carry a cwd).
       */
      function studioReadCwd(source) {
        return source === "bundled" ? "" : studioCwd(s);
      }

      /** The editor state for a freshly opened or created workflow (new session). */
      ...
      function openWorkflow(entry) {
        ...
        Archon.workflowDefinition(entry.name, studioReadCwd(source), source)
      ...
      function backToPicker() {
        ...
        nextSession(); // any Validate/Save reply still in flight is for the old session
        patch({
        ...
        Archon.validateDefinition(definitionFor(name))
          ...
          .then(function (res) {
            if (res && res.valid === true) {
              patchIfOpen(token, { busy: "", serverIssues: [], notice: "The server accepted this definition." });
              return;
            }
            if (!res || res.__error || res.__httpStatus < 200 || res.__httpStatus >= 300) {
              patchIfOpen(token, { busy: "", serverIssues: serverIssues([studioWriteError(res)]), notice: "" });
              return;
            }
            patchIfOpen(token, { busy: "", serverIssues: serverIssues(res.errors), notice: "" });
          });
      }

      /**
       * Save order, always: name check, client validation (both block before any
       * network call), then POST /workflows/validate, then PUT. `definition.name`
       * is forced to the filename the workflow is stored under.
       */
      function saveWorkflow(name, source) {
        // The name becomes a filename on the server, which rejects the shapes
        // isValidWorkflowName refuses with a 400. In-place Save reuses a name the
        // server already served or New already checked, but Save-as takes the
        // name straight from the form, so every save checks it here.
        if (!isValidWorkflowName(name)) { patch({ nameError: renameReasonMessage("invalid-name", name) }); return; }
            ...
            Archon.saveWorkflow(name, scope.cwd, scope.target, definition)
              ...
              .then(function (write) {
                if (!write || write.__error || write.__httpStatus < 200 || write.__httpStatus >= 300) {
                  patchIfOpen(token, { busy: "", notice: "Save failed: " + studioWriteError(write) + "." });
                  return;
                }
                // The file is written either way; only the editor-side bookkeeping
                // is skipped when the reply lands in a later session.
                refreshList();
      ...
      function reopenSaved(open) {
        Archon.workflowDefinition(open.name, studioReadCwd(open.source), open.source)
          .catch(function (e) { return { __error: e }; })
          .then(function (res) {
      ...
      function renameWorkflow(to) {
        ...
        Archon.saveWorkflow(to, scope.cwd, scope.target, definitionFor(to))
          ...
          .then(function (write) {
            if (!write || write.__error || write.__httpStatus < 200 || write.__httpStatus >= 300) {
              patchIfOpen(token, { busy: "", nameError: "Rename failed: " + studioWriteError(write) + "." });
              return;
            }
            ...
            Archon.deleteWorkflow(from, scope.cwd, scope.target)
              ...
              .then(function (removed) {
                ...
                if (session.id !== token) { refreshList(); return; } // the editor moved on; the files did not
                clearDirty({
                  busy: "",
      ...
      function deleteWorkflow() {
        ...
        Archon.deleteWorkflow(name, scope.cwd, scope.target)
          ...
          .then(function (res) {
            if (!res || res.__error || res.__httpStatus < 200 || res.__httpStatus >= 300) {
              patchIfOpen(token, { busy: "", armDelete: false, notice: "Delete failed: " + studioWriteError(res) + "." });
              return;
            }
            if (session.id !== token) { refreshList(); return; } // already elsewhere; just reflect the removal
            nextSession();
            clearDirty({
    ...
    function renderStudioIssues(s, h) {
      ...
      // Keys come from the issue itself, not the array index: the list is
      // re-ordered (errors first) and re-derived on every validation, so an
      // index key would hand one row's element to a different issue. Identical
      // issues (same severity, node and message) get a per-repeat suffix.
      var seen = newIdMap();
      function issueKey(issue) {
        var base = issue.severity + "|" + (issue.nodeId || "") + "|" + issue.message;
      ...
      return React.createElement("div", { className: "dsha-issues" },
        ...
        ordered.length === 0
          ...
          : React.createElement("ul", { className: "dsha-list dsha-issue-list" }, ordered.map(function (issue) {
            return React.createElement("li", { key: issueKey(issue) },
              React.createElement("button", {
                type: "button",
