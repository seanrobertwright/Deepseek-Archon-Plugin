...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function renderArtifacts(runId, p, togglePreview) {
      ...
      return React.createElement("div", { className: "dsha-detail-section" },
        React.createElement("h4", { className: "dsha-section-title" }, "Artifacts" + (files ? " (" + files.length + ")" : "")),
        body);
    }
    ...
    function RunDetailPanel(props) {
      ...
      React.useEffect(function () {
        ...
        Promise.all([
          ...
          patch({
            loading: false,
            error: detail.__error ? "Run detail unavailable: " + errorText(detail.__error) : "",
            run: detail.__error ? null : detail.run,
            events: !detail.__error && Array.isArray(detail.events) ? detail.events : [],
            artifacts: listingIsValid ? listing.files : null,
            artifactsError: listing && listing.__error
              ? "Artifacts unavailable: " + errorText(listing.__error)
              : (!listing || typeof listing !== "object" || !Array.isArray(listing.files)
                ? "Artifacts unavailable: invalid response from the server."
                : ""),
          });
        });
